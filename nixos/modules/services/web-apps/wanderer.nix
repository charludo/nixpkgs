    ...
    systemd.services.wanderer-web = {
      ...
      serviceConfig = {
        ...
        ExecStartPre =
          let
            aboutFile =
              if cfg.settings.about != null then pkgs.writeText "wanderer-about.md" cfg.settings.about else null;
          in
          pkgs.writeShellScript "wanderer-web-start-pre" # bash
            ''
              ${lib.optionalString (aboutFile != null) ''
                mkdir -p ${stateDir}/web-upper
                ${pkgs.unionfs-fuse}/bin/unionfs -o cow ${stateDir}/web-upper=RW:${cfg.package}/share/web=RO ${stateDir}/web
                mkdir -p ${stateDir}/web-upper/build/client/md
                ln -sfn ${aboutFile} ${stateDir}/web-upper/build/client/md/about.md
              ''}
            '';
